...
func TestTenantReconciler_Reconcile(t *testing.T) {
	tests := []struct {
		...
		{
			// TenantReconciler calls setupPortaClient (which calls CAProvider) as
			// the very first action after fetching the CR — no guards to bypass.
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := &capabilitiescontrollers.TenantReconciler{BaseReconciler: base, CAProvider: caProvider}
