...
// the expected number of deployment updates. Before the synthetic trigger this
// should be 0 (no perpetual reconcile during initial deployment). After the
// synthetic trigger it should be exactly 1 (the operator corrected the drift
// and stopped).
func verifyNoDeploymentUpdates(namespace, name string, expected int, w io.Writer) {
	...
	Expect(totalUpdates).To(Equal(expected),
		deploymentUpdateDetail(namespace, name, updateCounts, totalUpdates, expected))
}
...
func deploymentUpdateDetail(namespace, name string, counts map[string]int, total, expected int) string {
	var sb string
	sb = fmt.Sprintf("%s/%s: total deployment updates %d, expected %d; per-deployment breakdown:\n",
		namespace, name, total, expected)
